...
const { METHOD_CALL } = dbus.MessageType;
const { isObjectPathValid, isBusNameValid } = dbus.validators;
...
program
    ...
    .option("--variant-any", "Use any as Dbus.Variant type")
...
program.template = program.template || "typescript-class-auto.ts";
if (path.basename(program.template, ".hbs") === program.template) {
    program.template = path.resolve(__dirname, path.basename(program.template, ".hbs") + ".hbs");
}
...
const helpers = {
    ...
    varName(name) {
        if (["if", "else", "while", "class", "interface", "return"].includes(name)) {
            return name + "_";
        }
        return name;
    },
...
async function getInterfaceDataFromXml(data, objectPath) {
    ...
    const subNodes = (xml.node.node || []).map((n) => n["$"].name);
    ...
    const cleanXmlData = { ...xml, node: { ...xml.node } };
    delete cleanXmlData.node.node;
    const cleanData =
        '<!DOCTYPE node PUBLIC "-//freedesktop//DTD D-BUS Object Introspection 1.0//EN" "http://www.freedesktop.org/standards/dbus/1.0/introspect.dtd">\n' +
        builder.buildObject(cleanXmlData);

    return {
        interfaces: interfaces,
        subNodes: subNodes,
        xmlData: new Handlebars.SafeString(cleanData),
...
async function main() {
    ...
    if (program.recursive) {
        ...
        while (nodeList.length > 0) {
            ...
            nodeList.push(
                ...interfaceData.subNodes.map((name) => currentNode + (currentNode.endsWith("/") ? "" : "/") + name),
            );
