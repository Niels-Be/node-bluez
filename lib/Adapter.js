...
class Adapter extends DbusInterfaceBase {
    ...
    getDevice(address) {
        return this._bluez.getInterfaceInstance("org.bluez.Device1", Device, this._interface.objectPath, (d) => d.Address === address);
    }
    ...
    */
    SetDiscoveryFilter(filter) {
        return new Promise((resolve, reject) => {
            this._interface.SetDiscoveryFilter(filter, (err) => {
                if (err) return reject(err);
                resolve();
            })
        });
    ...
    */
    GetDiscoveryFilters() {
        return new Promise((resolve, reject) => {
            this._interface.GetDiscoveryFilters((err, filters) => {
                if (err) return reject(err);
                resolve(filters);
            })
        });
...
Adapter.INTERFACE_NAME = "org.bluez.Adapter1";
