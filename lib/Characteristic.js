...
class Characteristic extends DbusInterfaceBase {
    constructor(interface_, bluez) {
        super(interface_, bluez);

        this.onPropertyChanged = (intf, props, opts) => {
            if (intf !== Characteristic.INTERFACE_NAME) return;
    ...
    getDescriptor(uuid) {
        return this._bluez.findInterfaceInstance(Descriptor.INTERFACE_NAME, Descriptor, this._interface.objectPath, (d) => d.UUID === uuid);
    }
    ...
    StartNotify() {
        return new Promise((resolve, reject) => {
            this._interface.StartNotify((err) => {
                if (err) return reject(err);
                this.on('PropertiesChanged', this.onPropertyChanged)
                resolve();
            })
        });
    }
    ...
    StopNotify() {
        return new Promise((resolve, reject) => {
            this._interface.StopNotify((err) => {
                if (err) return reject(err);
                this.removeListener('PropertiesChanged', this.onPropertyChanged);
                resolve();
            })
        });
    }
...
Characteristic.INTERFACE_NAME = "org.bluez.GattCharacteristic1";
