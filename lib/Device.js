...
class Device extends DbusInterfaceBase {
    ...
    getService(uuid) {
        return this._bluez.findInterfaceInstance(Service.INTERFACE_NAME, Service, this._interface.objectPath, (d) => d.UUID === uuid);
    }
    ...
    */
    Connect(timeout) {
        return new Promise((resolve, reject) => {
            this._interface.Connect({timeout: timeout || -1}, (err) => {
    ...
    */
    ConnectProfile(uuid, timeout) {
        return new Promise((resolve, reject) => {
            this._interface.ConnectProfile(uuid, {timeout: timeout || -1}, (err) => {
                if (err) return reject(err);
                resolve();
            })
        });
...
Device.INTERFACE_NAME = "org.bluez.Device1";
